...
def main(args: List[str]) -> int:
    ...
    for key_code_num, key_info in numlock_off_keycodes.items():
        numlock_off_key_mapping += f'KeyInfo numlock_off_{key_info.name.lower()} {{true, &output_pins_state.{key_info.matrix[0].name}, &output_pins_state.{key_info.matrix[1].name}}};'
        numlock_off_key_mapping += f'     // {usb_key_name(key_info.name)} => {key_info.c128_name}\n'


    pins_set_array = ''
    for pin in Matrix:
        pins_set_array += f'    &output_pins_state.{pin.name},'
        pins_set_array += '\n'
